subroutine rhomeson(mp2,mf2,Zphi,etaphi,etapsi,h,l,lb,T,mu,k,omega,p,epsi,dgammarho_Redt,dgammarho_Imdt,dmrho2dt)
!This subroutine calculate the kurtosis analytically
  ...
  real(8) mp2,mf2,h,Zphi,etaphi,etapsi,g,T,mu,Zphi_p0
  ...
  dgammarho_Redt=-v3*h**2*k**2*((1.-etapsi)*fQL1_Re+etapsi*fQL2_Re)*Nc
  dgammarho_Imdt=-v3*h**2*k**2*((1.-etapsi)*fQL1_Im+etapsi*fQL2_Im)*Nc
  dmrho2dt=-32./105.*v3*h**2*Zphi*k**2*(7.-etaphi)*b3pion*0.              &
           +8.*Nc*v3*h**2*Zphi*k**2*(4.-etapsi)*(-5./36.*f2a+f3a/9.)   &
